...
The simulation data are in panels (a) and (b);
the fittings of simulation data with help Eqs.~(\ref{eqIF}) and (\ref{eqEef}) ($m_T = 1.13$, $m_B = 2.85$) are in panels (c) and (d).
}%
\end{figure}
...
\caption{\label{fig4}
Dependence of the parameter $n_0$ (see Eq.~(\ref{eqIF})) on the iron concentration in SC base.
FI--SRH case.
Line is the fitted curve using Eq.~(\ref{eqIF}).
}%
\end{figure}
...
FI--SRHBBA case.
The marks are the simulation results, and the lines are the fitted curves using Eq.~(\ref{eqEef}) and (\ref{eqEef}) ($m_T = 1.3$).
...
FI--SRHBBA  case.
Lines in panel (a) are the fitted curves using Eq.~(\ref{eqIF}), lines in panel (b) only
...
Lines are the fitted curves using Eq.~(\ref{eqIF})
}%
\end{figure}
