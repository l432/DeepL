...
So, the active ultrasound effects are observed in solar cells \cite{YOlikh2007TPL,Olikh2018JAP}, LEDs \cite{US:LED,UST:LED_SM}, and Schottky diodes \cite{Pashaev2014,Olikh:Ultras}.
...
The fitting of the $V>1$~V region by Eq.~(\ref{eqSDIV}) shown that the irradiation caused significant increase in both series resistance value and ideality factor  --- see Table~\ref{tabMIS} data.
...
Pintilie, \emph{et al}. \cite{FZSi:Rad} shown that complexes VO$_i$, Ñ$_i$C$_s$, $H$--centre (V$_2$O$_i$), $\Gamma$--centre, and interstitial defect $I^{0/-}$ are the main defects, created by $^{60}$Co $\gamma$-radiation in Si.
...
notably the trap concentration per unit energy range at energy $E$ above the valence band
...
where
$C$ is the constant,
$E_x$ is the activation energy that defined by the trap level position relative to the conductivity band.
