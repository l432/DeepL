...
However, it should be taken into account, that the displacement of the point defect without covalent bond could exceed a matrix atom displacement.
Finally, no US  absorption by defect is assumed.
...
At the same time, the dislocations are often located perpendicularly to $p-n$ junction plane in the SCR region,
...
whereas the axis of a pair of coupled defects with $(\Delta\Omega_d^\mathtt{D}\cdot\Delta\Omega_d^\mathtt{A}<0)$ should make a right angle with dislocation line.
...
Taking into account the experimental results and the estimation suggested by our model:
...
In our opinion, both donor and acceptor are defects of interstitial--type in non--irradiated sample, and one of pair components is a defect of vacancy--type in irradiated samples.
...
As expected, minority carrier lifetime increases as the temperature grows, and at 320~K,
...
As seen from the table, the estimated value of radiation--induced $\tau_n^{-1}$ change comprises ($8\div17$), 4, and 29~\% of
...
At the same time, the calculated lifetime changes $K_\tau\Psi$ are in quite good agreement with the changes expected from RDs production --- see Section~\ref{DefectType}.
...
As seen from the figure, the irradiation results in $R_{sh}$ decrease.
...
whereas in g6SC and g7SC the increase of $R_{sh}$ vs $T$  is close to linear at the vicinity of 293~K.
...
In the course of firing, Al particle can penetrate into the sample creating $p^+$--doped region around it, which compensates the emitter and remains in ohmic contact with the base.
...
Eq.~(\ref{eqRsh}) is true for simplified case of $K_p=K_n$.
...
$R_{293,\mathtt{Al}}$ does not depend on USL and increases as the irradiation level.
...
As a consequence, the number of Al particles grow, $R_{sh,\mathtt{Al}}$ decreases and becomes the key factor contributing in the overall shunt resistance.
...
Dispersion of $\sigma_{\mathtt{dis}}$ correlate with dispersion of$\tau_n$ over the samples set.
...
The non-irradiated sample was light soaked under illumination by halogen lamp (2 Suns) at approximately 305~K.
...
Over 5~h period, $I$--$V$ characteristics were measured with the interval 10—15 min in order to determine the kinetics of the parameters at room temperature.
...
Fig.~\ref{fig_Illum} shows the changes of structure parameters in comparison with those prior to illumination.
As seen from the figure, illumination does not result in considerable permanent change of $\tau_g$, $\tau_n$, $n_{\mathrm{id}}$ before as well as after annealing.
...
These facts allow us to conclude that the defect responsible for AI phenomena in nSC is mainly oxide precipitate.
...
The other defects that can be created by irradiation in silicon are I$_p$--center, bistable donor (BD), B$_i$O$_i$ and C$_i$C$_s$.
...
This effect is enhanced by alteration of AI divacancy in neutron--irradiated samples.
In other words, C$_i$O$_i$ is non--AA defect, whereas V$_2$ is AA defect.


In our opinion, under US action, $\tau_g$ and $n_\mathrm{id}$ in non--irradiated sample
...
VO$_2$ is formed after $300^\circ$C annealing of irradiated crystal,
...
Hence, under assumption made, VO$_i$ is a favorable pair for AI alteration of distance between component and therefore, can be transformed into metastable configuration by USL, which
causes changes in both $T_{\mathrm{id}}$ and $E_{\tau g}$.



\section{Conclusion}
The influence of ultrasound on $I$--$V$ characteristics of non-irradiated silicon $n^+$--$p$--structures as well as silicon structures exposed to reactor neutrons or $^{60}$Co gamma radiation have been investigated experimentally.
