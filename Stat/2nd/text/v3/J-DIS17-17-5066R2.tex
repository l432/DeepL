...
(iv)~reversibility of effect of low intensity AW.
...
However, ADI is also observed in such non--piezoelectric crystals as silicon, the basic material in microelectronic.
...
Defects are known to determine most of semiconductor devices characteristics.
...
as well as the US absorption by dislocation\cite{Davletova2008,OstrovKor92,Olikh:Ultras2016}
are believed to be the main mechanisms of elastic vibration--defect interaction in non--piezoelectric crystals.
...
The defects in silicon structures are not all acoustically active and can remain unmodified under the action of ultrasound.
The ADI efficiency depends on defect type and structure. \cite{UST:Medvid}
Thus, the force acting on the point defect in the crystal under US loading (USL) of crystal is determined by the relaxation of defect volume\cite{MirzadeJAP2011,PeleshchakUJF2016}.
...
For this purpose, the samples were irradiated by reactor neutrons and a $^{60}$Co--gamma source rays.
...
Our work presents distinction of AI effects in silicon structures with different RDs.
The intensity of applied US was insufficient to a new defect creation, a RDs annealing or a long distance (a many interatomic distance) diffusion.
As a result, the complete recovery of characteristics was observed after AW propagation had stopped.
To describe the processes in space charge region (SCR) and in the diode base as well as to study shunt resistance, we used the models of coupled defect level recombination,\cite{CDLR:JAP1995,CDLR:JAP} Shockley--Read--Hall (SRH) recombination and dislocation--induced impedance,\cite{Rsh:Gopal2003,Rsh:Gopal2004} respectively. The observed AI phenomena are accounted for in terms of defect interaction with AW strain field.\cite{MirzadeJAP2011,PeleshchakUJF2016}
...
$n^+$-$p$--Si structure was fabricated from 2~inch (300~$\mu$m thick) p--type boron doped Czochralski silicon wafer with $<$111$>$ orientation and resistivity of 10~$\Omega\cdot$cm.
...
To avoid the impact of  long--term annealing, which is typical to neutron damaged structure, \cite{NIEL:Moll,Rew:Srour} the irradiated samples were stored  for  5 years  at  room  temperature before the measurements.
...
We used Eqs. (\ref{eqIV})--(\ref{eqW}) to fit the experimental data taking $\tau_g$, $\tau_n$, $n_{\mathrm{id}}$, $R_{sh}$, and $R_s$ as the  fittings parameters.
Also, we used the known \cite{ni:Green,Schroder2006,Markvart} temperature dependences of $n_i$, $E_g$, and $\mu_n$.
In the result, we obtained extremely good fit to the experimental data --- see Fig.~\ref{figIV}.
In particular, for all the samples the value of $R_s$ was found to be about 1~$\Omega$.
The broken lines in Fig.~\ref{figIV}(a) show an example of the calculated contributions of $I_{SCR}$, $I_{base}$, and $I_{sh}$ to full current.

In case of USL , the transverse AWs with frequency of $4.2$~MHz, which were exited by using piezoelectric transducer,  were applied to the samples at the base side in the [111]--direction.
...
In order to wait the AI transitional period completion the following experimental procedure was used.
When USL started, the sample was first exposed to at room temperature for 60~min and then the $I$--$V$ measurement and the sample heating were started.
...
The parameters of $I$--$V$ characteristic associated with SCR phenomena are $n_{\mathrm{id}}$ and $\tau_{g}$.
...
As shown from the Fig.~\ref{fig_n}, the ideality factor decreases with temperature increase and the  dependence of $n_{\mathrm{id}}$ on $1/T$  is close to linear.
...
\end{equation}
The values of $T_{\mathrm{id}}$ and $E_{\tau g}$ found for both non--irradiated and irradiated samples under USL as well as without it are listed in Table~\ref{tabTpar}.
...
The phenomenon was first observed experimentally,\cite{DAPR:Chen1991,DAPR:Chen1994}  after which it was recruited to explain process in semiconductor diodes. \cite{CDLR:JAP1995,CDLR:JAP,CDLR:SSP}
...
In simplified case, when there is no carrier exchange between the donor level $E_t^{\mathtt{D}}$ and the valence band,
...
$\Delta\Omega_d > 0$ for the interstitial atoms and substitutional impurities with ionic radius exceeding the ionic radius of matrix
atoms, whereas
...
As seen from the table, the estimated value of radiation--induced $\tau_n^{-1}$ change comprise ($8\div17$), 4, and 29~\% of
its values measured for samples nSC, g6SC and g7SC, respectively, so this cannot explain the dispersion observed experimentally.
At the same time, the calculated lifetime changes $K_\tau\Psi$ are in quite a good agreement with those expected from RDs production --- see Section~\ref{DefectType}.
...
The resistance temperature coefficient was estimated from g7SC data.
...
Obtained values of $R_{293,\mathtt{Al}}$ and $\sigma_{\mathtt{dis}}$ are given in Table~\ref{tabTpar}.
...
The irradiation facilitates to the formation of vacancies as well as Al diffusion out of the electrodes.
...
which result in lifetime changes.
...
With regard to SCR recombination, in our opinion, under US action, $\tau_g$ and $n_\mathrm{id}$ in non--irradiated sample
