...
We used Eqs. (\ref{eqIV}) and (\ref{eqW}) to fit the experimental data and the $I_{ph}$ (for illuminated $I$--$V$ curves only), $\tau_g$, $\tau_n$, $n$, $R_sh$, and $R_s$ were taken as the  fittings parameters.
...
It should be noted that the short-circuit current density $J_{sc}\approx J_{ph}R_{sh}/(R_{sh}+R_{s})$.
...
Therefore $J_{sc}\approx J_{ph}$.
The open-circuit voltage $V_{oc}$ and the fill factor $FF$ were estimated from illuminated $I$--$V$ curves by the conventional mode.
