...
The investigation has revealed the acoustically driven reversible increment of an ideality factor and decrement of a carrier lifetime both in the space charge region and in the solar cell base, a a photocurrent, an open--circuit voltage,  a fill factor, and a shunt resistance.
...
It has been given evidence of the degradation is due to the recombination amplification under the ultrasound wave action.
...
The qualitative model of ultrasound influence, which is based on a change of a distance between defects or between components of defect complex under an alternating deformation action, is proposed.
It has been shown that the oxide precipitates are likely defects, which takes part in a acousto--defect interaction.
...
The additional investigations are necessary to define a type of defects, which are involved in both an recombination and a acousto--defect interaction.
...
Annealing had an more efficient influence on SSC parameters and results in  an increment of $\tau_g$ and $R_{sh}$ as well as in a decrement of $\tau_n$ and $n$.
...
As one can see, $\tau_n$ and $R_{sh}$ does not vary practically.
Hence these parameters can are be influenced by iron--boron pairs.
at the same time  $\tau_g$ and $n$ are changing after illumination. 
We supposed that the expressions, that describes the both $\tau_g$ and $n$ evolution, are similar to    Eq.~(\ref{eqFeB}) and used the characteristic time $\left[1.3\cdot10^{-3}(1.4\times10^{15})^{\,2/3}\exp\left(-\frac{0.68}{295k}\right)\right]^{-1}=2.53\cdot10^4$~s to plot fitting line on \Fref{figAfter}(a) and (b).
...
Hence, on our opinion, the pairs Fe$_i$B$_s$ are not main reason of a SCR recombination in the investigated samples.
...
Dislocations and stacking faults surround  the oxide precipitates and lead to capture coefficient change as well as to increase the concentrations of the two defects, without introducing additional states into the bandgap \cite{MurphySC2014,MurphyJAP2011,MurphyJAP2012} and can provide their acousto--activity.
The oxide precipitates are nonuniformly distributed across a Cz--Si wafer \cite{Oxide_Schon} or a solar cell \cite{Oxide:Chen} and it is probable reason for a SSC parameters variation from one investigated sample to another.
On the basis of mentioned above, we conclude that the defects, involved in both an recombination and a acousto--defect interaction are oxide precipitates mainly.
...
We taken $\alpha$ temperature dependence from \cite{Markvart} (p.39, Eq.~(18)), and supposed, that $\beta$ and $M$ were temperature independent, $L_n\sim T^{0.5}$.
