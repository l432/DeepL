...
was chosen to avoid
The light  wavelength of $\lambda=900$~nm ensured the electron–-hole pair generation in the $p$-region mainly and low light intensity of $W_{ph}=2$~W/m$^2$ was chosen to avoid any LID processes.
...
\subsection{Ultrasound influence on the recombination in the SCR}
...
The extent of acoustically induced effect is weakly depend on both ultrasound intensity and wave type and becomes more apparent in the sample with a low $R_{sh}$ value.
The longitudinal and transverse USL differed by $f_{U\!S}$ too, but it was shown previously \cite{Olikh2011Sem,Olikh:Ultras2016}, that ultrasound influence on the silicon structures had arose with wave frequency.
$f_{U\!S}$ of the longitudinal USL (L-USL) is greater than in the case of transverse wave using (T-USL) , but  T-USL is more effective (see \Fref{figDUS}, Table~\ref{tab}).
Therefore the crucial factor of the ultrasound influence on the SSC parameters is the wave type.


\begin{figure}
\begin{center}
\includegraphics[width=15cm]{olikhFig2abcd}%
\end{center}
\caption{\label{figDUS}
Temperature dependences of SCR carrier lifetime (a), ideality factor (b), base  carrier lifetime (c) and shunt resistance (d) with (curves 2--4 and 6--8) and without (1 and 5) USL of the SC-R15 (squares, curves 1--4) and SC-R4 (circles, curves 5--8).
The longitudinal (2, 6) and transverse (3, 4, 7, 8) ultrasound waves were used.
$W_{U\!S}$=0.40~W/cm$^2$ (2,6);
...
\begin{table}
\caption{\label{tab}Acoustically induced changes of SSC parameters.}
\begin{center}
%\begin{ruledtabular}
...
At the same time, the experimentally observed large $n$ and small $\tau_{g}$ can be explained by the model of coupled defect level recombination \cite{CDLR:JAP,CDLR:JAP1995}.
...
\begin{eqnarray}
\label{eqSigma}
\sigma_{n,p} = \sigma_{n,p}^0\left(\frac{r}{r_0}\right)^2\,,\\
\label{eqRda}
r_{DA} = c_{DA}^0N_DN_AL(r)=c_{DA}^0N_DN_A\left[1+\frac{r}{a_0}+\frac{1}{3}\left(\frac{r}{a_0}\right)^2\right]\exp\left(-\frac{r}{a_0}\right)\,,
\end{eqnarray}
where
...
$r_0$ and $c_{DA}^0$ are some constant.
...
Unfortunately, the functional relation between ideality factor and DAP recombination parameters is absent,
but $n$ is shown \cite{CDLR:JAP1995,CDLR:SSP} to increase with the $r$ and $r_{DA}$ decrement and with the $\sigma_{n,p}$ increment.

The capture cross section is known \cite{Bube,Sachenko} to depend on temperature as $T^{-1}$ up to $T^{-3}$ for the attractive centers.
On our opinion, the decrease of $\sigma_{n}^{0,D}$ and $\sigma_{p}^{0,A}$ determines the observed temperature dependencies of both $n$ and $\tau_{g}$.


On the other hand, according to \cite{MirzadeJAP2011,PeleshchakUJF2016}, the force acting on the point defect during USL can be expressed as
\begin{equation}
\label{eqFd}
...
$\partial \xi(x,t)/\partial x$ linear depend on the deformation amplitude $\xi_0=\sqrt{2W_{U\!S}/\rho_{Si}\,\vartheta_{U\!S}^3}$ 
(where $\rho_{Si}=2.33$~g/m$^3$ is the silicon density, $\vartheta_{U\!S}$ is the ultrasound velocity, $\vartheta_{U\!S}^L=9850$~m/s,  $\vartheta_{U\!S}^T=5840$~m/s).
Therefore, a point defect oscillates under USL condition and oscillation amplitude and phase are determined by both defect character and acoustic wave intensity and type.
