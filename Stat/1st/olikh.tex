...
\Fref{figDUS}(c) shows the USL influence on the minority carrier lifetime in the p--region of SSC.
One can see that $\tau_n$ decreases down to 20\% of its initial value under ultrasound action.
The relative change of an electron lifetime $\varepsilon_{\tau n}$ is presented in Table~\ref{tab}.

Ordinary the lifetime reduction under radiation action is described by the Messenger–-Spratt equation \cite{Markvart}:
\begin{equation}
\label{eqMS}
\frac{1}{\tau_n}=\frac{1}{\tau_{n0}}+K\Phi\,,
\end{equation}
where $\tau_{n0}$ is the minority carrier lifetime in the unirradiated cell, 
$\Phi$f is the integrated flux, 
and $K$ is a lifetime damage constant characteristic for the material and the type of irradiation.
The dependencies of reciprocal lifetime on ultrasound intensity is shown on \Fref{figMS}.
As one can recognize, the linear dependence of $1/\tau_n$ on $W_{{U\!S}}$  is observed.
Therefore the modified Messenger–-Spratt equation can be used to describe the AI change of the lifetime:
\begin{equation}
\label{eqMS_Us}
\frac{1}{\tau_n}=\frac{1}{\tau_{n0}}+K_{U\!S}W_{{U\!S}}\,,
\end{equation}
where $\tau_{n0}$ is the lifetime without USL condition and
$K_{U\!S}$ characterizes ultrasound influence and depends on acoustic wave type and sample state.
...
\end{center}
\caption{\label{figMS}
Dependencies of reciprocal electron lifetime in the SSC base on ultrasound intensity for SC-R4 (circles, left axis) and SC-R15 (squares, right axis) at 320~K.
Symbol filling is defined by the intensity and type of ultrasound waves and coincides with \Fref{figDUS} notation.
...
Hence the observed reversible decrement in lifetime is evident of an acoustically driven change of electron capture cross section.
