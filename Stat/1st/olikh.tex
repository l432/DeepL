...
The extent of acoustically induced effect is weakly depend on both ultrasound intensity and wave type and becomes more apparent in the sample with a high $R_{sh}$ value.
...
Let's consider a simplest model, which is shown on \Fref{figModel}.
Two point defect, donor (D) and acceptor (A) are situated at distance $r_0$.
...
$\varphi$ is the angle between the defect pair axis and the forced displacement.

Generally,  the distance $r$ between donor and acceptor would not equal to $r_0$ during vibration.
\Fref{figR2L} and \Fref{figLnew} show relative change of capture cross section $\varepsilon_{\sigma}=[\sigma(r)-\sigma(r_0)]/\sigma(r_0)$ and coupling parameter $\varepsilon_{r_{AD}}=[r_{AD}(r)-r_{AD}(r_0)]/r_{AD}(r_0)$.
...
the amplitude dependencies of $\varepsilon_{\sigma}$ and $\varepsilon_{r_{AD}}$ are determined by $|u_D-u_A|$ and $(u_D+u_A)$ respectively. In particular
\begin{equation}
\label{eqEpsSig}
\varepsilon_{\sigma}=\frac{(u_D\pm u_A)^2}{2r_0^2}\,.
\end{equation}

According to the \Fref{figR2L}(a) and Eq.~(\ref{eqEpsSig}), the capture cross section increases under USL condition.
Acoustically induced alteration of coupling parameter is more complicated and sign of $\varepsilon_{r_{AD}}$ depend on $\varphi$ and $r_0$.
