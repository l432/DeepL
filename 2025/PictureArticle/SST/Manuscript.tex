...
There are two main reasons for the prospects of a similar investigation.
...
The control tools for similar defects are necessary.
This study aimed to investigate the impact of MWT on the defects located in the near-surface region
...
iv)~Epitaxial $n$-$n^+$-$n^{++}$ structures of GaAs:Te with a buffer layer.
 They were made from a single-crystal (100) substrate (300~$\mu$m, $n= 2\times10^{18}$~cm$^{-3}$)
  ...
  Two samples (GAB1 and GAB2) were cut from different wafers.

Epitaxial systems have been produced by using gas-phase epitaxy.
The schematic structure of the samples under study are shown in \fref{figSamp_TAV}.
...
The  principle of this spectroscopy closely coincides with the idea  of  the DLTS
technique \cite{TAV:1993MIS},
but the  barrier formation is not required,
...
The summary of the trap parameters detected in the samples is presented in \tref{tabMW}.
...
In particular, \tref{tabMW} provides  data for the GAT (GAS) and GAB series samples after 20~s of MWT,
...
Generalizing the above-mentioned, we should note that it is the energy level that
...
In addition, the connection between the level $E_c-0.26$~eV and the carbon vacancy
...
the equation~\eref{eqBeta} is correct in the case of
...
It should be noted that similar effect of the decrease in the decoration of dislocations
after low fluence neutron radiation
is reported \cite{LightNeuIrrad:1,LightNeuIrrad:2} as well.
However, the suggested \cite{LightNeuIrrad:1,LightNeuIrrad:2} reason is
...
\mathrm{V}_\mathrm{Si}\;\mathrm{V}_\mathrm{C}+\mathrm{Si}_{\,i} \rightarrow
   \mathrm{V}_\mathrm{C}\,.
\end{equation*}
In the case of SIC3 sample, the defect transformation occurred at a low dose
...
Similar processes occur in a GaAs single-crystal.
...
On the other hand, the weaker efficiency of MWT influence in the sample
...
\end{eqnarray}
At the same time, the high density of dislocation at the near-surface
...
In addition, the growth of epitaxial layers is known to lead to
...
\end{equation*}
for GAB1 and GAB2.
EGA6 corresponds to the final defect configuration in GAE1 and GAE2 samples.
...
The similar effect of the change in ionization energy
...
The possible defect reactions have been discussed.
