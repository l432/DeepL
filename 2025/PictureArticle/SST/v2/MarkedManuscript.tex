...
Temperature influences the characteristic time of FeB complex association (see}  \colorbox{yellow}{Eq.~\eref{eqTass}),}
\hl{
...
As an example,}  \colorbox{yellow}{\Fref{FigN}a and \Fref{FigN}b }
\hl{
show the wavelet spectrograms for two distinct iron concentrations.
Comparison with}  \colorbox{yellow}{\Fref{Fig2}b and \Fref{Fig2}c}
\hl{
...
suggesting a corresponding increase in model prediction accuracy.}
 \colorbox{yellow}{\Fref{FigN}c }
\hl{presents the MAPE for models trained and tested on synthetic datasets prepared in the same manner as those used previously,
but under 270 K conditions.
More detailed metrics are provided in Figures S4 and S5 of the Supplementary Materials.
Comparison with}  \colorbox{yellow}{\Fref{Fig6}}
\hl{
(lower right panel) confirms that a notable improvement in model performance is observed.
}}
...
Calculations based on }\colorbox{yellow}{Eq.~\eref{eqTass}}
\hl{
...
(see} \colorbox{yellow}{\tref{tabSuplFigs}),}
\hl{
...
%https://www.mediafire.com/file/qplaq2uy77td9ko/SupplementaryMaterial.pdf/file

\begin{table*}
\centering
\caption{\color[rgb]{1.00,0.07,0.00}{\colorbox{yellow}{Overview of Supplementary Figures S1--S10$^*$.}}} \label{tabSuplFigs}
\begin{indented}
\item[]
{\color[rgb]{1.00,0.07,0.00}
\colorbox{yellow}{
...
%\begin{table*}
%\centering
%\caption{Overview of Supplementary Figures S1--S10.} \label{tabSuplFigs2}
%\begin{indented}
%\item[]
%\begin{tabular}{p{2.2cm} p{4.0cm} p{4.6cm} p{4.6cm}}
%\br
%Figures & Data source and setup & Content & Purpose / key message \\
%\mr
%S1, S8 &
%Training on simulated (S1) and experimental (S8) datasets &
%Scatter plots of $N_{\mathrm{Fe,PRED}}$ versus $N_{\mathrm{Fe,TRUE}}$ for all 87 model variants for training and test phases &
%Visual assessment of prediction accuracy and systematic deviations \\
%
%S2--S3 &
%Training on simulated dataset &
%Heatmaps of MSE, $R^2$, MAPE, and MdAPE for all CV--regressor combinations (training and test) &
%Baseline comparison and ranking of all 87 model variants \\
%
%S4--S5 &
%Training on simulated dataset ($T = 270$~K) &
%Heatmaps of performance metrics under temperature-specific simulation conditions &
%Evaluation of model robustness at low temperature \\
%
%S6--S7 &
%Test on experimental dataset (without / with post-hoc calibration) &
%Heatmaps of performance metrics &
%Impact of post-hoc calibration on experimental predictions \\
%
%S9--S10 &
%Training on experimental dataset &
%Heatmaps of performance metrics (training and test) &
%Consistency of model performance when trained on experimental data \\
%\br
%\end{tabular}
%\end{indented}
%\end{table*}
