...
%До певної міри з точністю пов'язане питання щодо застосовності створених моделей beyond the chosen range of $N_\mathrm{B}$, $d_p$, and $T$.
%
%Хочемо подякувати Рецензенту за цікаве запитання.
%Насамперед, при його розгляді необхідно окремо розглядати кожен з параметрів.
%Здатність моделей робити передбачення beyond the range of training dataset залежить від характеру взаємозв'язку  target variable and descriptors.
%Як було показано раніше [], співвідношення між відносними змінами фотоелектричних параметрів та концентрацією заліза можуть суттєво змінюватися залежно від величин температури та концентрації легуючої домішки в базі. Це пов'язано з тим, що від температури та концентрації носіїв залежать низка параметрів як кремнію (bandgap, carrier mobilities, коефіціенти іntrinsic recombination, положення рівня Фермі тощо), так і iron-related дефектів (carrier capture cross-sections).
%Ці взаємозв'язки, які враховувалися при створенні тренувального набору, є достатньо складними, особливо стосовно Nb, про що свідчать результати, отримані для Nb-altered набору.
%Як вже зазначалося, в цьому випадку навіть з задачею інтерполяції всередині використаного при тренування range of Nb вдалося задовільно впоратися лише
%найбільш потужним з точку зору здатності виявлення схованих зв'язків DNN моделям.
%Задача прогнозування beyond the training range of Nb є ще більш складною і тому на нашу думку можливості створених моделей достатньо обмежені.
%
%Пов'язані з температурою взаємозв'язки простіші, про що свідчать непогані результати отримані при використанні tree-based для T-altered набору (див. рис.9, Таблиця 3).
%Проте для цей тренувальний набір містив значення температури, які знаходилися beyond the training range і в цьому випадку, як видко з Рис.10, таких моделей суттєво знижувалась.
%Фактично знову ж таки, найкращі усереднені результати спостерігаються для DNN-моделей (див.рис.9) і саме при їхньому використанні можна очікувати більш-менш точних прогнозів
%beyond the training range of температури.
%
%З шириною бази ситуація простіша, так як її вплив на вигляд залежностей Nfe від змін PVPs набагато слабший і часто має лінійних характер [].
%Тому на нашу думку можна очікувати застосовності створених моделей для передбачень за межами chosen in the study range.
%Звичайно, розширення діапазону dp можливе лише в певних межах, поки суттєво не зміниться відношення цієї величини до довжини дифузії неосновних носіїв.
%
%Узагальнюючи, зазначимо, що за необхідності отримання прогнозів для випадків,
%які знаходяться beyond the range значень параметрів, використаних під час тренування,
%доцільно застосовувати  лише DNN-моделі. При цьому вимоги до можливих відхилень значень товщини бази найменш строгі,
%тоді як для температури відхилення навряд чи можуть перевищувати 10 К.
%Використання розглянутих моделей при відмінних від використаних значеннях  концентрації легуючої домішки не рекомендується.
%Фактично, це є додатковою умовою, якій має задовольняти тренувальний набір, щоб точність прогнозів моделі була задовільною.
%Принагідно зауважимо, в нашому дослідженні були вибрані діапазони параметрів, які  closely reflect those of real solar cells and their operating (testing) conditions.
%
%Обговорення щодо можливостей застосування моделей beyond the selected range of parameters долучена до Subsection 3.7 (останні 4 параграфи).
%
%
% а для концентрації легуючої домішки в базі
%температури достадля отримання достатньої точності очікувана точність прогнозів при
...
%\hl{}}
%
%{\colorbox{yellow}{\textcolor[rgb]{1.00,0.07,0.00}{}}
