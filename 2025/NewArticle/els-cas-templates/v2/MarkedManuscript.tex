...
including short-circuit current ($I_\mathrm{SC}$), open-circuit voltage ($V_\mathrm{OC}$), efficiency ($\eta$), and fill factor ($F!F$).
...
leads to variations in PVPs that depend on iron concentration ($N_\mathrm{Fe}$).
...
achieving results comparable to the best outcomes obtained for the $N_\mathrm{Fe}$--altered and $T$--ltered datasets.
