...
However, in the last decade, deep learning, which are enable to solve problems without clear algorithmization, have been successfully used in various fields of theoretical and applied physics \cite{MachLean_RevModPhys,MachLeanJAP,MachLeanPPV}. 
This gives hope for an real implementation of aforesaid SC characterization method with deep learning approaches using (so to say "deep learning for deep levels").
...
Iron is a major contaminant as well as one of the most detrimental
metal impurities in solar--grade crystalline silicon materials \cite{HowMuchPhysics,FeB:Schmidt}.
A simple back surface field (BSF) $n^+$--$p$--$p^+$ structure are important from an applied point of view.

The deep learning is based on learn by examples.
In this work the set of examples were simulated by using SCAPS-1D \cite{SCAPS1,SCAPS2}, which is widely used to model silicon-based devices \cite{SCAPSuseSi4,SCAPSuseSi1,SCAPSuseSi6} as well.
