...
In the second one, the $\mathrm{Fe}_i$ was suggested to be presence only with uniform distribution.
...
The base thickness $d_p$ ($150-240$~$\mu$m) and doping level (boron concentration, $N_B=10^{15}-10^{17}$~cm$^{-3}$) changed from one simulation to another.
...
(for $T$ and $d_p$ in linear scale, for $N_{\mathrm{Fe}}$ and $N_B$ in logarithmic scale) 
distributed in the mentioned ranges.
Besides some test labeled dataset were prepared.
For example, $d_p$, $N_{\mathrm{Fe}}$ and $N_B$ values are equal to values of training dataset and $T$ differs from them
for one of test dataset, which labeled "T-varied".
The values of $d_p$, $N_{\mathrm{Fe}}$ and $N_B$ are different from training dataset mesh in "d-varied", 
