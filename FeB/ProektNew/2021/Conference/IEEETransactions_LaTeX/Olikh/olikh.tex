...
set of DNN input parameters (DNN--sample) consisted of $\{d_p,N_\mathrm{B},T,n_\mathrm{Fe-FeB}\}$.
...
In practical terms, the obtaining such a set requires additional SC processing (e.g. intense illumination) and two IV measuring.
...
The loss function was chosen mean square relative error:
\begin{equation}
\label{eqMSRE}
    MSRE=\frac{1}{N_s}\sum_{i=1}^{N_s}\frac{(N_\mathrm{Fe,TRUE,i}-N_\mathrm{Fe,PRED,i})^2}{N_\mathrm{Fe,TRUE,i}\cdot N_\mathrm{Fe,PRED,i}}\,,
\end{equation}
where
...
\caption{Results of 10--Fold Cross--Validation Of Used DNN}
\label{table_V}
\centering
\begin{tabular}{lcc}
\hline
...
and part of samples for which the SRE does not exceed a certain value (blue lines).
$n_\mathrm{Fe-FeB}$ DNN and $n_\mathrm{Fe-FeB}$--$n_\mathrm{Fe}$ DNN, learnt by training dataset, are used
in panels (a)--(e) and (g)--(k), respectively.
Test dataset: T-varied (a, g),
d-varied (b, h),
B-varied (c, i),
Fe-varied (d, j),
All-varied (e, k).
Panels (f) and (l) represent data from
...
DNN, Learnt by Training Dataset, on Test Dataset}
\label{table_MSRE}
\centering
\begin{tabular}{lcc}
\hline
...
and trained DNN are presented at https://github.com/olegolikh/IVcharacteristics.git.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{olikh_foto}}]{Oleg Olikh}
 %Oleg Ya. Olikh
