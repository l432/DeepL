...
SC parameters ($d_p$ and $N_\mathrm{B}$),  IV measurement temperature, and result of IV fitting (ideality factor value).
As it is shown in Fig.~\ref{fig_chem} two DNNs with different input parameters were under consideration.
The input sample of the first one consist of $\{d_p,N_\mathrm{B},T,n_\mathrm{Fe-FeB}\}$.
...
The second one uses  $\{d_p,N_\mathrm{B},T,n_\mathrm{Fe-FeB},n_\mathrm{Fe}\}$ in input layer.
