...
The popularity of O2DM is related to the fact that experiments typically measure IV curves only in the fourth quadrant 
(from short-circuit current to open-circuit voltage). 
The algorithms considered here have different inspiration sources. 
Some of these algorithms have been well-known for a long time 
and have proven their effectiveness in solving a wide range of problems. 
Other algorithms are more recent, developed with the knowledge gained from their predecessors. 
The results are compared using diverse nonparametric statistical methods.
%The O2DM popularity is also associated with the fact that in experiments,
...
Research has shown that using a squared error-based fitness function offers clear advantages in tackling the given problem. 
The overall performance results of various algorithms generally align with the NFL. 
GOTLBO, PSO, ISCA, NNA, CWOA, and WW are unsuitable for parameter estimation in the opposed two-diode model. 
DE, NDE, MABC, and IJAYA yield better results than the previous group, 
but using these algorithms for parameter identification is not recommended. 
EBLSHADE and TLBO are generally effective in accurately determining parameters, 
but they may make mistakes under certain conditions. 
Thus, their application for solar cells with S-shaped IV curves should be approached cautiously. 
STLBO and ADELI have shown superior performance in accuracy and reliability compared to other algorithms. 
Particularly, these two algorithms successfully determine parameters from similar IV curves of photovoltaic cells with distinct characteristics.

%Research has demonstrated that utilizing a squared error-based fitness function offers clear advantages in tackling a provided problem.
%The overall performance results of various algorithms generally fit the NFL.
