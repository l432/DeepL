...
\begin{table*}[<options>]
\caption{Adjusted p-values for multiple comparisons $1\times N$ tests in single-IV case (ADELI is the control algorithm, $R_\mathrm{p1}$ evaluation task}\label{tbl1NADELI}
...
\begin{figure}[]
	\centering
	  ...
	  \caption{The results of Friedman, Friedman Aligned, and Quade tests in the IV-set case.
               The control algorithms are in rows.
               Each colored hexagon indicates that the adjusted p-value does not exceed 0.1
               for every post-hoc procedure.
               The shaded hexagon indicates that the adjusted p-value does not exceed 0.1
               for the Finner procedure only.
