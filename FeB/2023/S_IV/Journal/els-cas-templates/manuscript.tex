...
or a combination of these cases.
In this study, the primary focus was on the accuracy of parameter estimation,
and therefore, the termination criterion used was the absence of changes in the solution.
...
\toprule
Method  &  $N_\mathrm{it}$ & $N_\mathrm{FE}$ & $t_\mathrm{run}$ (s)\\ % Table header row
\midrule
...
%перекласти на англійську "Ячейка з символом SE свідчить, що точність алгоритму, вказаному у рядку, щодо визначення параметра, вказаного у колонці, при використанні функції Аф перевищує результати, отримані з використанням функції Ив. Ячейка з символои АА свідчить про кращі результати Аф. У випадку символу = is no significant difference between АА та ВА"
...
\toprule
Method  & $I_{01}$& $n_1$ & $R_\mathrm{p1}$ &$I_{02}$& $n_2$& $R_\mathrm{p2}$&$R_\mathrm{s}$&$I_\mathrm{ph}$&RMSPE\\ % Table header row
\midrule
