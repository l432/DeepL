...
to extract the SC parameters from synthetic IV curves, which were generated with a range of parameter values. 
The obtained results have been compared using nonparametric statistical methods, 
including Wilcoxon signed-rank test for pairwise comparisons, Friedman, Friedman Aligned, and Quade tests for multiple comparisons. 
Comprehensive results and analyses indicate that STLBO and ADELI algorithms have highly competitive performance in terms of accuracy and reliability.
\end{abstract}
...
According to the International Energy Agency reports, the cumulative installed PV capacity
...
is to use an equivalent circuit model.
Such lumped-parameter modeling allows simulation, analysis, and optimization of device performance.
Currently, silicon solar cells (Si-SCs) make up approximately 90\% of the global PV production capacity.
For describing the operation of such conventional SCs, single-diode model (SDM), double-diode
model (DDM), and three-diode model (TDM) are most commonly used.
...
The PV module model (PVMM) is based on SDM and is made up of several diodes in series and/or parallel.
In addition to developing models, identifying the relevant parameters from current-voltage characteristics (IVCs)
...
\cite{IMPA,SCA_NM_OBL,HFAPS,IWOA} or SDM, DDM, and TDM \cite{Kepler}.
A comprehensive survey of meta-heuristic algorithms for parameter
extraction of conventional PV models can be found in \cite{Li2021,Yang2020}.
Moreover, the utilization of conventional SC models for testing newly developed
algorithms is among the most popular approaches after using benchmark functions of the Congress on Evolutionary Computation.

The variety of algorithms used for the analysis of conventional models is associated with the existence of the No Free Lunch (NFL) theorem \cite{NFL}.
NFL states that no single algorithm can solve all optimization tasks effectively.
...
Recently, intensive research is underway for innovative developments,
further experiments, and practical PV applications of various new-generation SCs (NG-SCs).
The operating principle of NG-SCs is qualitatively similar to Si-SCs,
but the fourth quadrant of IVC often exhibits an S-shaped kink.
The kink origin has been attributed to various physical phenomena
and such IVC feature is typical in the most promising candidates for the NG-SCs.
...
Thus, new models are proposed to give some reasonable IV curve shape explanations from the view of electricity.
However, the NFL theory suggests that algorithms that have demonstrated exceptional results for Si-SC models
may not yield the same effectiveness for NG-SC models.
Thus, the relevant task arises of choosing metaheuristic algorithms for extracting parameters from S-shaped IV curves.
...
The novelty of this work is associated with determining the metaheuristic algorithm most appropriate for extracting the parameters of NG-SCs.


Today, a considerable number of models have been proposed with the aim of explaining S-shaped IV curves
...
Another way to develop equivalent models involves using multiple series diodes.
...
%\label{eqParSingleIV}
%\begin{split}
%I_{01}&=1.6\cdot10^{-6}\,\text{mA},\quad n_1=1.92,\quad R_\mathrm{p1}=190\,\Omega,\\
%I_{02}&=0.16\,\text{mA},\quad n_2=1.92, \quad R_\mathrm{p2}=190\,\Omega,\\
%R_\mathrm{s}&=45\,\Omega,\quad I_\mathrm{ph}=8\,\text{mA},
%\end{split}
%\end{equation}
\begin{equation}
\label{eqParSingleIV}
