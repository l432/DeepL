...
Наступним кроком після розробки моделі є визначення відповідних параметрів з ВАХ. 
Така задача по своїй суті є однією проблемою оптимізації.
Одним з популярних шляхів вирішення подібних задач є застосування метаеврістичних алгоритмів.
Загалом таких алгоритмів розроблено дуже багато. 
Підгрунтям для такого різноманіття є No Free Lunch theory (NFL), яка стверджує, що no metaheuristic algorithm is able to solve all optimization tasks.
In other words, the optimization algorithm can have brilliant results in a specific class of problems and at the same time fails to solve other classes of problems.
Як наслідок, кожна проблема, зокрема оцінка параметрів кожної еквівалентної моделі, потребує індивідуального підбору алгоритму.
З літератури відомо чимало, робіт, які присвячені порівнянню ефективності метаеврістичних алгоритмів щодо визначення параметрів відповідно до загальноприйнятих моделей сонячних елементів. У деяких з них в центрі уваги перебуває одна з моделей (наприклад SDM, DDM або TDM), деякі одночасно розглядають SDM та DDM.
У багатьох роботах розглянуто ефективність метаеврістичних алгоритмів у задачі обробки ВАХ відповідно до трьох моделей: SDM, DDM та PVMM.
Зокрема, використання моделей сонячних елементів для тестування новостворених алгоритмів є чи ненайбільш популярним після використання CEC test functions. 
Різноманіття метаеврістичних методів оптимізації дозволяє реалізуватися ситуації, коли різні роботи сфокусовані на різних наборах алгоритмів.
...
Метою даного дослідження було порівняння ефективності визначення параметрів відповідно до дво-діодної моделі Кастро для набору з 14 метаеврістичних алгоритмів та визначення кращого з них. Вибір саме цієї моделі з усього різноманіття зумовлений тим що вона є достатньо універсальною. Незважаючи на вказані вище її недоліки у першому квадранті,
...
%\cite{Roland2016}
%
%So-called S-kink current vs. voltage (J-V) curves have been reported
%for solar cells based on numerous different materials and architectures,
%including but not limited to devices based on organic materials, chalcogenides including CIGS and CdTe, and passivated c-Si heterojunction devices.
%
