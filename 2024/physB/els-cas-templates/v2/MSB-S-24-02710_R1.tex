...
Simulations covered temperatures (290–340 K), iron concentrations ($10^{10}$--$10^{14}$~cm$^{-3}$), and SSC base thicknesses (180--380~$\mu$m) and
and boron doping ($10^{15}$--$10^{17}$~cm$^{-3}$) under AM1.5 and monochromatic (940~nm) illumination.
Experimental results confirmed simulation predictions.
The study assessed the ability to use photoelectric parameter variations for iron quantification, identifying short-circuit current under monochromatic light as a highly reliable metric.
The study correlations between photoelectric characteristics suggested that Principal Component Analysis is advisable for dimensionality reduction when SSC multiple parameters are used in non-invasive iron detection.
\end{abstract}
...
%• Iron defect dynamics on Si solar cells performance analyzed via SCAPS simulations (81)
%• Short-circuit current changes are most suitable for iron concentration estimation (81)
%• Open-circuit voltage changes are non-monotonic with iron concentration at low doping (84)
%• Using monochromatic illumination improves iron concentration estimates over AM1.5 (81)
%• PCA eliminates redundant PV data, enhancing ML-based iron concentration estimation (82)
...
\begin{figure}
	\centering
	  ...
	  \caption{Schematic diagram of analyzed solar cells.
      he right side of the figure illustrates the configurations of interstitial iron Fe$_i$ and iron-boron pairs Fe$_i$B$_s$.}\label{fig1}
\end{figure}
...
Furthermore, it has been demonstrated that iron-induced changes in various photovoltaic parameters are correlated.
Therefore, if iron concentration estimation is performed using changes
in multiple photovoltaic parameters, applying Principal Component Analysis is advisable.
